...
import demoji
from underthesea.pipeline.text_normalize import (
    text_normalize as _underthesea_text_normalize,
)
...
class TextNormalizePipeline:
    def text_normalize(self, text):
        text = _underthesea_text_normalize(demoji.replace(text))
        return text
    ...
    def process_item(self, item, spider):
        ...
        if adapter.get("address")["full_address"]:
            adapter["address"]["full_address"] = self.text_normalize(
                adapter.get("address")["full_address"]
            )
        return item
...
class AddressCorretionPipeline:
    def __init__(self):
        self.province_name_map = {
            ...
            "tp-ho-chi-minh": "TP. Hồ Chí Minh",  # run.sh uses this key
            "ho-chi-minh": "Hồ Chí Minh",  # run_new.sh uses this key
            "long-an": "Long An",
        ...
        self.address_correcter = geoapivietnam.Correct(
            use_fuzzy=True, print_result=True
        )
        province_distric = os.path.join(
            os.path.dirname(os.path.dirname(__file__)),
            "province_district_ward_prefix.json",
        )
        with open(province_distric, "r", encoding="utf8") as f:
    ...
    def correct_district(self, full_address, province):
        if full_address:
            ...
            district = self.address_correcter.correct_district(
                province=province, district=full_address
            )
        else:
            district = None
    ...
    def full_name_with_prefix(self, province=None, district=None, ward=None):
        if ward:
            prefix = self.province_district_ward_prefix[province][district][ward][
                "prefix"
            ]
    ...
    def get_administrative_unit_from_address(self, address):
        ...
        def get_province_from_raw_address(address, prefix=False):
            for province in self.province_district_ward_prefix:
                if province == "prefix":
                    continue
                match_province = (
                    self.full_name_with_prefix(province) if prefix else province
                )
            ...
            if found_province:
                for district in self.province_district_ward_prefix[found_province]:
                    if district == "prefix":
                        continue
                    match_district = (
                        self.full_name_with_prefix(found_province, district)
                        if prefix
                        else district
                    )
                ...
                for province in self.province_district_ward_prefix:
                    ...
                    for district in self.province_district_ward_prefix[province]:
                        if district == "prefix":
                            continue
                        match_district = (
                            self.full_name_with_prefix(province, district)
                            if prefix
                            else district
                        )
                        if match_district.lower() in address.lower():
        ...
        def get_ward_from_raw_address(
            address, found_province=None, found_district=None, prefix=False
        ):
            if found_district:
                for ward in self.province_district_ward_prefix[found_province][
                    found_district
                ]:
                    if ward == "prefix":
                        continue
                    match_ward = (
                        self.full_name_with_prefix(found_province, found_district, ward)
                        if prefix
                        else ward
                    )
                ...
                if found_province:
                    for district in self.province_district_ward_prefix[found_province]:
                        ...
                        for ward in self.province_district_ward_prefix[found_province][
                            ...
                            if ward == "prefix":
                                continue
                            match_ward = (
                                self.full_name_with_prefix(
                                    found_province, district, ward
                                )
                                if prefix
                                else ward
                            )
                    ...
                    for province in self.province_district_ward_prefix:
                        ...
                        for district in self.province_district_ward_prefix[province]:
                            if district == "prefix":
                                continue
                            for ward in self.province_district_ward_prefix[province][
                                district
                            ]:
                                if ward == "prefix":
                                    continue
                                match_ward = (
                                    self.full_name_with_prefix(province, district, ward)
                                    if prefix
                                    else ward
                                )
                                if match_ward.lower() in address.lower():
        ...
        if address["province"]:
            ...
            province = get_province_from_raw_address(
                address["full_address"], prefix=True
            )
            if province == None:
                province = get_province_from_raw_address(
                    address["full_address"], prefix=False
                )
        # Get province and district by matching address string and real name
        province, district = get_district_from_raw_address(
            address["full_address"], province, prefix=True
        )
        if district == None:
            province, district = get_district_from_raw_address(
                address["full_address"], province, prefix=False
            )
        # Get province and district by matching address string and real name
        province, district, ward = get_ward_from_raw_address(
            address["full_address"], province, district, prefix=True
        )
        if ward == None:
            province, district, ward = get_ward_from_raw_address(
                address["full_address"], province, district, prefix=False
            )
    ...
    def process_item(self, item, spider):
        ...
        if adapter.get("address")["full_address"]:
            adapter["address"]["full_address"] = adapter.get("address")[
                "full_address"
            ].strip()
            province, district, ward = self.get_administrative_unit_from_address(
                adapter.get("address")
            )
            if district == None:
                district = self._remove_prefix(
                    self.correct_district(
                        adapter.get("address")["full_address"], province
                    ),
                    self.all_prefixs,
                )
    ...
    def price_preprocess(self, item):
        ...
        if price is None:
            ...
            if "thỏa thuận" in price:
                ...
                if (
                    len(price_split) == 2
                    and price_split[0].replace(".", "", 1).isnumeric()
                ):
                    if price_split[1] == "tỷ":
                        multiplicity = 1e9
...
class DuplicateCheckPipeline:
    ...
    def _is_similar(self, saved, candidate):
        ...
        similarity = cosine_similarity(
            saved_description_vector, candidate_description_vector
        ).item()

        is_similar = (
            saved_title == candidate_title and similarity > 0.95
        ) or similarity > 0.99
    ...
    def process_item(self, item, spider):
        ...
        candidate = {
            ...
            "description_vector": self.vectorizer.transform(
        ...
        for real_estate in self.real_estates:
            is_similar, candidate_description_vector = self._is_similar(
                saved=real_estate, candidate=candidate
            )
            if is_similar:
                raise DropItem(
                    f"This real estate has already been collected at {real_estate['post_id']}"
                )
...
class PushToKafka:
    ...
    def __init__(self, kafka_bootstrap_servers, topic, stats):
    ...
    def process_item(self, item, spider):
        ...
        # Trigger callbacks for previously produced messages
    ...
    def _delivery_report(self, err, msg):
        ...
        if err is not None:
            self.stats.inc_value("kafka/delivery_failed")
            print("Message delivery failed: {}".format(err))
        else:
            self.stats.inc_value("kafka/delivery_success")
